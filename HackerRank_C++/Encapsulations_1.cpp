...
class Words {
  public:
    ...
    void CoutStrings() {
       ...
       vector<int> non_vowel_lengths;
       int size;
       int length;
       for (auto a : list_of_words) {
         size = CountVowels(a, a.length());
         vowel_sizes.push_back(size);
         length = CountNonVowels(a, a.length());
         non_vowel_lengths.push_back(length);
       }
       /*
      ...
      */
      for (int i = 0; i < non_vowel_lengths.size(); i++) {
         if (i == non_vowel_lengths.size()-1) {
           cout << non_vowel_lengths.at(i) << endl;
         }
         else {
           cout << non_vowel_lengths.at(i) << ',';
         }
       }
     }
  ...
  private:
    ...
    int CountNonVowels(string str, int n) {
      if (n == 1) {
        return NotVowel(str[n-1]);
      }
      else {
        return CountNonVowels(str, n-1) + NotVowel(str[n-1]);
      }
    }
};
