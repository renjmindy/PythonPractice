...
def findCrossoverIndexHelper(x, y, left, right):
    ...
    if len(x) <= 2:
        ...
        mid = (right - left + 1) // 2
        d = x[mid] - y[mid]
        if (x[mid-1] - y[mid-1]) * d < 0:
            return mid-1
        else:
            if (x[0] - y[0]) * d > 0:
                return mid + findCrossoverIndexHelper(x[mid:], y[mid:], 0, len(x)-1-mid)
            elif (x[0] - y[0]) * d < 0:
...
# YOUR ANSWER HERE : (left)^3 <= j^3 <= n and n < (j + 1)^3 <= right^3, so we have left < j + 1 and j < right. left <= j < right
...
# YOUR ANSWER HERE : n * ((k-1) + (k-2) + (k-3) + ... + 2 + 1) = n * k * k / 2 = n * k^2 / 2
